...
module.exports = {
  data: new SlashCommandBuilder().setName("알림목록").setDescription("현재 서버에 등록된 알림 목록을 조회합니다."),
  ...
  async execute(interaction, reminderManager) {
    ...
    const lines = reminders.map((reminder, index) => {
      return [
        ...
        `- 유형: ${reminder.type === "WEEKLY" ? "주간 반복" : "특정 날짜 1회"}`,
        `- 스케줄: ${reminderManager.formatSchedule(reminder)}`,
    ...
    if (content.length > 1900) {
      const summary = reminders
        .map((reminder, index) => {
          return `${index + 1}. \`${reminder.id}\` | ${
            reminder.type === "WEEKLY" ? "주간" : "1회"
          } | ${reminderManager.formatSchedule(reminder)} | ${reminderManager.formatDateTime(reminder.nextRunAt)}`;
        })
        .join("\n");
